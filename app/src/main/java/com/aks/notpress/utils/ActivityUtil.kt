...
class ActivityUtil {
    ...
    private fun startActivity(activity: Activity, event: ActivityStartEvent) {
        val intent = when (event.type) {
            ...
            OPEN_PROMO_CODE      -> Intent(Intent.ACTION_VIEW, Uri.parse("https://play.google.com/redeem?code="))
...
enum class ActivityType(val code: Int) {
    ...
    OPEN_YOUTUBE(104), VIDEO(105), OPEN_SETTING(106), OPEN_BOOK(107), OPEN_PROMO_CODE(108)
}